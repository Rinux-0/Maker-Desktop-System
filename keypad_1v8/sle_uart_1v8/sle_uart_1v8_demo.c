...
#if defined(CONFIG_SAMPLE_SUPPORT_SLE_UART_SERVER_1V8)
#include "securec.h"
#include "sle_uart_server.h"
#include "sle_uart_server_adv.h"
#include "sle_device_discovery.h"
#include "sle_errcode.h"
#elif defined(CONFIG_SAMPLE_SUPPORT_SLE_UART_CLIENT_1V8)
#define SLE_UART_TASK_STACK_SIZE            0x600
#include "sle_connection_manager.h"
#include "sle_ssap_client.h"
#include "sle_uart_client.h"
#endif  /* CONFIG_SAMPLE_SUPPORT_SLE_UART_CLIENT_1V8 */
...
uint32_t rx_length = SLE_UART_SERVER_MSG_QUEUE_MAX_SIZE;
uint8_t sle_connect_state[] = "sle_dis_connect";
#endif /* CONFIG_SAMPLE_SUPPORT_SLE_UART_SERVER_1V8 */



void sle_uart_1v8_demo_init(void) {
    #if defined(CONFIG_SAMPLE_SUPPORT_SLE_UART_CLIENT_1V8)
    /* UART pinmux. */
    uart_init_pin();
    ...
    uapi_uart_unregister_rx_callback(CONFIG_SLE_UART_BUS);
    errcode_t ret = uapi_uart_register_rx_callback(CONFIG_SLE_UART_BUS, UART_RX_CONDITION_FULL_OR_IDLE, 1, sle_uart_client_read_int_handler);
    sle_uart_client_init(sle_uart_notification_cb, sle_uart_indication_cb);

    if (ret != ERRCODE_SUCC) {
        osal_printk("Register uart callback fail.");
    }
    #elif defined(CONFIG_SAMPLE_SUPPORT_SLE_UART_SERVER_1V8)
    sle_uart_server_create_msgqueue();
    sle_uart_server_register_msg(sle_uart_server_write_msgqueue);
    sle_uart_server_init(ssaps_server_read_request_cbk, ssaps_server_write_request_cbk);
    ...
    uapi_uart_unregister_rx_callback(CONFIG_SLE_UART_BUS);
    errcode_t ret = uapi_uart_register_rx_callback(CONFIG_SLE_UART_BUS, UART_RX_CONDITION_FULL_OR_IDLE, 1, sle_uart_server_read_int_handler);
    if (ret != ERRCODE_SUCC) {
        osal_printk("%s Register uart callback fail.[%x]\r\n", SLE_UART_SERVER_LOG, ret);
    }
    #endif /* CONFIG_SAMPLE_SUPPORT_SLE_UART_CLIENT_1V8 */
...
void sle_uart_1v8_demo_oneloop(void) {
    #ifdef CONFIG_SAMPLE_SUPPORT_SLE_UART_SERVER_1V8
    sle_uart_server_rx_buf_init(rx_buf, &rx_length);	led_toggle();mdelay(500);
    sle_uart_server_receive_msgqueue(rx_buf, &rx_length);	led_toggle();mdelay(500);
    if (strncmp((const char*)rx_buf, (const char*)sle_connect_state, sizeof(sle_connect_state)) == 0) {
        errcode_t ret = sle_start_announce(SLE_ADV_HANDLE_DEFAULT);	led_toggle();mdelay(500);
        if (ret != ERRCODE_SLE_SUCCESS) {
            osal_printk("%s sle_connect_state_changed_cbk,sle_start_announce fail :%02x\r\n", SLE_UART_SERVER_LOG, ret);	led_toggle();mdelay(500);
        }
    }
    osal_msleep(SLE_UART_TASK_DURATION_MS);	led_toggle();mdelay(500);
    #endif /* CONFIG_SAMPLE_SUPPORT_SLE_UART_SERVER_1V8 */
}



void sle_uart_1v8_demo_exit(void) {
    #ifdef CONFIG_SAMPLE_SUPPORT_SLE_UART_SERVER_1V8
    sle_uart_server_delete_msgqueue();
    #endif /* CONFIG_SAMPLE_SUPPORT_SLE_UART_CLIENT_1V8 */
}
