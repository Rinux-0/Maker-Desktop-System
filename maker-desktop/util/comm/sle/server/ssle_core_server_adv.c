...
static u16 sle_set_adv_local_name(u8* adv_data, u16 max_len) {
    errno_t ret;
    u8 index = 0;
...
static u16 sle_set_adv_data(u8* adv_data) {
    size_t len = 0;
    u16 idx = 0;
    errno_t  ret = 0;
...
static u16 sle_set_scan_response_data(u8* scan_rsp_data) {
    u16 idx = 0;
    errno_t ret;
