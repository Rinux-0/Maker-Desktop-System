...
static char g_sle_uuid_app_uuid[SLE_UUID_LEN_2] = { 0x12, 0x34 };                       /* sle server app uuid for test */
static char g_sle_property_value[SLE_OCTET_BIT_LEN] = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 }; /* server notify property uuid for test */
...
static void sle_uuid_set_base(sle_uuid_t* out) {
    ...
    if (ret != EOK) {
        LOG("%s sle_uuid_set_base memcpy fail\n", SLE_SERVER_LOG);
        out->len = 0;
        return;
    }
...
static void sle_uuid_setu2(u16 u2, sle_uuid_t* out) {
    ...
    out->len = SLE_UUID_LEN_2;
    encode2byte_little(&out->uuid[SLE_UUID_INDEX], u2);
}


static void sle_uuid_print(sle_uuid_t* uuid) {
    if (uuid == NULL) {
        LOG("%s uuid_print,uuid is null\n", SLE_SERVER_LOG);
        return;
    }
...
static errcode_t sle_ssaps_register_cbks(ssaps_read_request_callback ssaps_read_callback, ssaps_write_request_callback
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        LOG("%s sle_ssaps_register_cbks,ssaps_register_callbacks fail :%x\n", SLE_SERVER_LOG, ret);
        return ret;
    }
...
static errcode_t sle_uuid_server_service_add(void) {
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        LOG("%s sle uuid add service fail, ret:%x\n", SLE_SERVER_LOG, ret);
...
static errcode_t sle_uuid_server_property_add(void) {
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        LOG("%s sle add property fail, ret:%x\n", SLE_SERVER_LOG, ret);
        osal_vfree(property.value);
        return ERRCODE_SLE_FAIL;
    }
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        LOG("%s sle add descriptor fail, ret:%x\n", SLE_SERVER_LOG, ret);
        osal_vfree(property.value);
...
static errcode_t sle_server_add(void) {
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        LOG("%s sle add service fail, ret:%x\n", SLE_SERVER_LOG, ret);
        return ERRCODE_SLE_FAIL;
    }
...
errcode_t sle_server_send_report_by_uuid(const u8* data, u8 len) {
    ...
    if (param.value == NULL) {
        LOG("%s send report new fail\n", SLE_SERVER_LOG);
        return ERRCODE_SLE_FAIL;
    }
    if (memcpy_s(param.value, param.value_len, data, len) != EOK) {
        LOG("%s send input report memcpy fail\n", SLE_SERVER_LOG);
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        LOG("%s sle_server_send_report_by_uuid,ssaps_notify_indicate_by_uuid fail :%x\n", SLE_SERVER_LOG, ret);
        osal_vfree(param.value);
        return ret;
    }
    osal_vfree(param.value);
...
static void sle_connect_state_changed_cbk(u16 conn_id, const sle_addr_t* addr,
    ...
    LOG("%s connect state changed callback conn_id:0x%02x, conn_state:0x%x, pair_state:0x%x, disc_reason:0x%x\n", SLE_SERVER_LOG, conn_id, conn_state, pair_state, disc_reason);
    LOG("%s connect state changed callback addr:%02x:**:**:**:%02x:%02x\n", SLE_SERVER_LOG, addr->addr[SLE_BT_INDEX_0], addr->addr[SLE_BT_INDEX_4]);
...
static void sle_pair_complete_cbk(u16 conn_id, const sle_addr_t* addr, errcode_t status) {
    ...
    LOG("%s pair complete addr:%02x:**:**:**:%02x:%02x\n", SLE_SERVER_LOG, addr->addr[SLE_BT_INDEX_0], addr->addr[SLE_BT_INDEX_4]);
    g_sle_pair_hdl = conn_id + 1;
...
static errcode_t sle_conn_register_cbks(void) {
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        LOG("%s sle_conn_register_cbks, sle_connection_register_callbacks fail :%x\n", SLE_SERVER_LOG, ret);
        return ret;
    }
    return ERRCODE_SLE_SUCCESS;
}
...
static void ssaps_server_r_request_cbk(u8 server_id, u16 conn_id, ssaps_req_read_cb_t* read_cb_para, errcode_t status) {
    LOG(
...
static void ssaps_server_w_request_cbk(u8 server_id, u16 conn_id, ssaps_req_write_cb_t* write_cb_para, errcode_t status) {
    LOG(
...
errcode_t sle_server_set_r_cb(sle_r_cb_t r_cb) {
    return sle_ssaps_register_cbks((ssaps_read_request_callback)r_cb, ssaps_server_w_request_cbk);
}


errcode_t sle_server_core_init(void) {
    errcode_t ret = sle_conn_register_cbks();
    if (ret != ERRCODE_SLE_SUCCESS) {
        LOG("%s sle_server_core_init, sle_conn_register_cbks fail :%x\n", SLE_SERVER_LOG, ret);
        return ret;
    }
    ret = sle_ssaps_register_cbks(ssaps_server_r_request_cbk, ssaps_server_w_request_cbk);
    if (ret != ERRCODE_SLE_SUCCESS) {
        LOG("%s sle_server_core_init, sle_ssaps_register_cbks fail :%x\n", SLE_SERVER_LOG, ret);
        return ret;
    }
    ret = sle_server_add();
    if (ret != ERRCODE_SLE_SUCCESS) {
        LOG("%s sle_server_core_init,sle_server_add fail :%x\n", SLE_SERVER_LOG, ret);
        return ret;
    }
