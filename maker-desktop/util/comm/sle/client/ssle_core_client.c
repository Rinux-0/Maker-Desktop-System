...
u16 sle_client_conn_id[SLE_CLIENT_MAX_CON] = { 0 };
...
void sle_client_notification_cb(u8 client_id, u16 conn_id, ssapc_handle_value_t* data,
	errcode_t status) {
	unused(client_id);
	unused(conn_id);
	unused(status);
...
void sle_client_indication_cb(u8 client_id, u16 conn_id, ssapc_handle_value_t* data,
	errcode_t status) {
...
static void sle_client_seek_result_info_cbk(sle_seek_result_info_t* seek_result_data) {
	...
	if (seek_result_data != NULL) {
		if (sle_client_conn_num < SLE_CLIENT_MAX_CON) {
...
static void sle_client_connect_state_changed_cbk(u16 conn_id, const sle_addr_t* addr,
...
void  sle_client_pair_complete_cbk(u16 conn_id, const sle_addr_t* addr, errcode_t status) {
...
static void sle_client_exchange_info_cbk(u8 client_id, u16 conn_id, ssap_exchange_info_t* param,
	errcode_t status) {
...
static void sle_client_find_structure_cbk(u8 client_id, u16 conn_id,
...
static void sle_client_find_property_cbk(u8 client_id, u16 conn_id,
	ssapc_find_property_result_t* property, errcode_t status) {
	osal_printk("%s sle_client_find_property_cbk, client id: %d, conn id: %d, operate ind: %d, " "descriptors count: %d status:%d property->handle %d\r\n", SLE_CLIENT_LOG, client_id, conn_id, property->operate_indication, property->descriptors_count, status, property->handle);
	sle_client_send_param.handle = property->handle;
	sle_client_send_param.type = SSAP_PROPERTY_TYPE_VALUE;
}


static void sle_client_find_structure_cmp_cbk(u8 client_id, u16 conn_id,
...
static void sle_client_write_cfm_cb(u8 client_id, u16 conn_id,
