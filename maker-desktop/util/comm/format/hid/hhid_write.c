...
static hid_ch9329_pack hid_wp;	// 写包



/// @brief 获取芯片版本等信息 -HID_CH9329_CMD_GET_INFO
static void hid_set_wp_cmd0x01(void) {
	hid_wp.length	= 0;
	hid_wp.sum		= 3;	// 基础值
}
...
static void hid_set_wp_cmd0x02(u8 data_len, const u8* data, const s16* data_map) {
	hid_wp.length	=  8;
	hid_wp.sum		= 12;	// 基础值

	hid_wp.data[0] = 0;		// 修饰键：预先置0
	hid_wp.data[1] = 0;		// CH9329要求置0
		...
		for (u8 j=0; j<8; j++)					// 每寄存器，逐bit
			if (data[i] & (1<<j)) {				// 按下
				if (data_map[i*8 + j] < 0)		// 修饰键
					hid_wp.data[0] |= 1 << (~data_map[i*8+j] - 1);
...
static void hid_set_wp_cmd0x05(u8 data_len, const u8* data, const s16* data_map) {
...
static void hid_set_wp_cmd0x06(u8 data_len, const u8* data, const s16* data_map) {
...
static void hid_set_wp_cmd0x0F(void) {
...
void hid_set_wp(u8 cmd, u8 data_len, const u8* data, const s16* data_map) {
	...
	break;case HID_CH9329_CMD_SEND_KB_GENERAL_DATA:
		hid_set_wp_cmd0x02(data_len, data, data_map);
	break;case HID_CH9329_CMD_SEND_MS_REL_DATA:
		hid_set_wp_cmd0x05(data_len, data, data_map);
	break;case HID_CH9329_CMD_SEND_MY_HID_DATA:
		hid_set_wp_cmd0x06(data_len, data, data_map);
