...
static const u8 UART_BUS_ID(u8 bus_id) {
	switch (bus_id) {
	default:		return 3;
...
static const u8 UART_TXD_PIN(u8 bus_id) {
	switch (bus_id) {
...
static const u8 UART_RXD_PIN(u8 bus_id) {
	switch (bus_id) {
	default:		return PIN_NONE;
...
static const u32 UART_BAUDRATE(u8 bus_id) {
	switch (bus_id) {
	default:		return 0;
	break;case 0:	return UART_0_BAUDRATE;
	break;case 1:	return UART_1_BAUDRATE;
	break;case 2:	return UART_2_BAUDRATE;
	}
}
...
static void uart_init_cfg(u8 bus_id) {
	uart_attr_t basic_attr = {
		.baud_rate	= UART_BAUDRATE(bus_id),
...
void uart_sle_r_int_handler(u8 cs_id, u16 conn_id, ssle_ssap_value_t* read_cb_para, errcode_t status) {
	...
	uart_write(UART_1_BUS_ID, read_cb_para->data, read_cb_para->data_len);
	#elif defined(CONFIG_COMM_SLE_SERVER)
	uart_write(UART_1_BUS_ID, read_cb_para->value, read_cb_para->length);
	#endif
}
